...
public record ServerboundDockyardRepairPacket(BlockPos pos, boolean hull, boolean sails) implements ModPacket {
    public static final ResourceLocation ID = ModPackets.id("server_dockyard_repair");

    public static ServerboundDockyardRepairPacket read(FriendlyByteBuf buf) {
        return new ServerboundDockyardRepairPacket(buf.readBlockPos(), buf.readBoolean(), buf.readBoolean());
    }
    ...
    public void write(FriendlyByteBuf buf) {
        ...
        buf.writeBoolean(this.hull);
        buf.writeBoolean(this.sails);
    }
