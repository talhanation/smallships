...
public class DockyardBlockModel {
	...
	/**
	 * Blockbench exports entity models upside down, so the model has to be flipped before it can be
	 * used for a block. Both the block entity renderer and the item renderer go through here, that
	 * way the item in hand is guaranteed to look exactly like the placed block.
	 *
	 * @param yRot rotation of the block in degrees, taken from its facing property
	 */
	public void render(PoseStack poseStack, VertexConsumer vertexConsumer, int packedLight, int packedOverlay, float yRot) {
		poseStack.pushPose();
