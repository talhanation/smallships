...
public class ChainShotRenderer extends EntityRenderer<ChainShotEntity>{
    /** spin speed of the chain shot in degrees per tick (moderate tumble, ~1.4 turns/s) */
    ...
    public void render(@NotNull ChainShotEntity entity, float entityYaw, float partialTicks, PoseStack poseStack, MultiBufferSource bufferIn, int packedLightIn) {
        ...
        // continuous spin: tickCount accumulates, partialTicks only smooths the
        // step between two ticks (a partialTicks-only term resets every tick and
        // therefore barely rotates at all)
    ...
    @Override
    public boolean shouldRender(@NotNull ChainShotEntity entity, @NotNull Frustum frustum, double camX, double camY, double camZ) {
